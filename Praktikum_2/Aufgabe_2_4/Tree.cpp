...
Tree::Tree(): letzte_id(0), anker(nullptr)
{
...
Tree::~Tree()
{
    std::cout << "call Tree::~Tree()" << std::endl;
...
{
    anker = nullptr;
    std::cout << "call Tree::Tree(const Tree& t)" << std::endl;
    t.traversePreorder([&](const TreeNode* node)
    {
        this->addNode(node->getName(), node->getAlter(), node->getPLZ(), node->getEinkommen());
    });
}

const Tree & Tree::operator=(Tree t)
{
    std::cout << "call Tree::operator=(Tree t)" << std::endl;
    swap(*this, t);
    return *this;
}
...
void Tree::swap(Tree & t1, Tree & t2)
{
    std::swap(t1.letzte_id, t2.letzte_id);
    std::swap(t1.anker, t2.anker);
}

void Tree::traversePreorder(std::function<void(const TreeNode*k)> callback) const
{
    traversePreorder(this->anker, callback);
}
...
void Tree::traversePreorder(const TreeNode *k, std::function<void(const TreeNode *)> callback) const
{
    if (k == nullptr)
        return;
...
void Tree::traverseInorder(const TreeNode *k, std::function<void(const TreeNode*)> callback) const
{
    if (k == nullptr)
        return;
...
void Tree::traversePostorder(const TreeNode *k, std::function<void(const TreeNode*)> callback) const
{
    if (k == nullptr)
        return;
